 ...
 */
public class WineProfilesVintageAdapter extends BaseAdapter {

    private static final int FIRST_ROW = 0;

    private static final int ALL_YEARS_OFFSET = 1;
    ...
    @Override
    public int getCount() {
        return mWineProfiles.size() + ALL_YEARS_OFFSET; //+1 for the all years row
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (row == null) {
            ...
            row = (ChooseVintageDialogRow) inflater
                    .inflate(R.layout.row_dialog_choose_vintage_impl, parent, false);
        }

        if (position == FIRST_ROW) {
            row.updateData(mBaseWine);
        } else {
            position -= ALL_YEARS_OFFSET; //adjust position so show the correct wine profile data
