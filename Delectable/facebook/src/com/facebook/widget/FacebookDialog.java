...
public class FacebookDialog {
     ...
     * @param appCall     an PendingCall containing the call ID and original Intent used to launch
     *                    the dialog
     ...
     * @param features zero or more features to check for; {@link ShareDialogFeature#SHARE_DIALOG}
     ...
     * version of the Facebook application; false if any of the features are not supported
     */
    ...
    /**
     * Determines whether the version of the Facebook application installed on the user's device is
     * recent enough to support specific features of the native Message dialog, which in turn may be
     * used to determine which UI, etc., to present to the user.
     *
     * @param context  the calling Context
     * @param features zero or more features to check for; {@link com.facebook.widget.FacebookDialog.MessageDialogFeature#MESSAGE_DIALOG}
     *                 is implicitly checked if not explicitly specified
     * @return true if all of the specified features are supported by the currently installed
     * version of the Facebook application; false if any of the features are not supported
     */
     ...
     *
     * @param context  the calling Context
     * @param features zero or more features to check for; {@link OpenGraphActionDialogFeature#OG_ACTION_DIALOG}
     *                 is implicitly checked if not explicitly specified
     * @return true if all of the specified features are supported by the currently installed
     * version of the Facebook application; false if any of the features are not supported
     */
    public static boolean canPresentOpenGraphActionDialog(Context context,
        ...
        return handleCanPresent(context,
                EnumSet.of(OpenGraphActionDialogFeature.OG_ACTION_DIALOG, features));
    }

    /**
     * Determines whether the version of the Facebook application installed on the user's device is
     * recent enough to support specific features of the native Open Graph Message dialog, which in
     * turn may be used to determine which UI, etc., to present to the user.
     *
     * @param context  the calling Context
     * @param features zero or more features to check for; {@link com.facebook.widget.FacebookDialog.OpenGraphMessageDialogFeature#OG_MESSAGE_DIALOG}
     *                 is implicitly checked if not explicitly specified
     * @return true if all of the specified features are supported by the currently installed
     * version of the Facebook application; false if any of the features are not supported
     */
    public static boolean canPresentOpenGraphMessageDialog(Context context,
        ...
        return handleCanPresent(context,
                EnumSet.of(OpenGraphMessageDialogFeature.OG_MESSAGE_DIALOG, features));
    }
    ...
    private static int getProtocolVersionForNativeDialog(Context context, String action,
        ...
        return NativeProtocol
                .getLatestAvailableProtocolVersionForAction(context, action, requiredVersion);
    }
     ...
     * Facebook application. As additional features are added, these flags may be passed to {@link
     * FacebookDialog#canPresentShareDialog(android.content.Context, com.facebook.widget.FacebookDialog.ShareDialogFeature...)}
     * to determine whether the version of the Facebook application installed on the user's device
     * is recent enough to support specific features, which in turn may be used to determine which
     * UI, etc., to present to the user.
     */
    public enum ShareDialogFeature implements DialogFeature {
        /**
         * Indicates whether the native Share dialog itself is supported by the installed version of
    ...
    /**
     * Defines a set of features that may be supported by the native Message dialog exposed by the
     * Facebook Messenger application. As additional features are added, these flags may be passed
     * to {@link FacebookDialog#canPresentMessageDialog(android.content.Context,
     * com.facebook.widget.FacebookDialog.MessageDialogFeature...)} to determine whether the version
     * of the Facebook application installed on the user's device is recent enough to support
     * specific features, which in turn may be used to determine which UI, etc., to present to the
     * user.
     */
    public enum MessageDialogFeature implements DialogFeature {
        /**
         * Indicates whether the native Message dialog itself is supported by the installed version
    ...
    /**
     * Defines a set of features that may be supported by the native Open Graph dialogs exposed by
     * the Facebook application. As additional features are added, these flags may be passed to
     ...
     * version of the Facebook application installed on the user's device is recent enough to
     * support specific features, which in turn may be used to determine which UI, etc., to present
     * to the user.
     */
     ...
     * exposed by the Facebook application. As additional features are added, these flags may be
     * passed to {@link FacebookDialog#canPresentOpenGraphMessageDialog(android.content.Context,
     * com.facebook.widget.FacebookDialog.OpenGraphMessageDialogFeature...)} to determine whether
     * the version of the Facebook application installed on the user's device is recent enough to
     * support specific features, which in turn may be used to determine which UI, etc., to present
     * to the user.
     */
    public enum OpenGraphMessageDialogFeature implements DialogFeature {
         ...
         * installed version of the Messenger application.
         */
    ...
    public interface Callback {
         ...
         *                    launch the dialog
         * @param data        a Bundle containing the results of the dialog, whose contents will
         *                    vary depending on the type of dialog being displayed.
         */
    ...
    abstract static class Builder<CONCRETE extends Builder<?>> {
        ...
        /**
         * Sets the name of the application to be displayed in the dialog. If provided, this
         * optimizes the user experience as a preview of a shared item, etc., can be displayed
         * sooner.
         *
        ...
        /**
         * Sets the fragment that should launch the dialog. This allows the dialog to be launched
         * from a Fragment, and will allow the fragment to receive the {@link
         * Fragment#onActivityResult(int, int, android.content.Intent) onActivityResult} call rather
         * than the Activity.
         *
         ...
         * dialog within the Facebook application.
         *
        ...
        public FacebookDialog build() {
            ...
            Intent intent = NativeProtocol
                    .createPlatformActivityIntent(activity, action, protocolVersion, extras);
            if (intent == null) {
        ...
        /**
         * Determines whether the native dialog can be presented (i.e., whether the required version
         * of the Facebook application is installed on the device, and whether the installed version
        ...
        protected List<String> addImageAttachments(Collection<Bitmap> bitmaps) {
            ...
            for (Bitmap bitmap : bitmaps) {
                ...
                String url = NativeAppCallContentProvider
                        .getAttachmentUrl(applicationId, appCall.getCallId(),
                                attachmentName);
    ...
    private abstract static class ShareDialogBuilderBase<CONCRETE extends ShareDialogBuilderBase<?>>
         ...
         * fatal and cause the dialog to return an error
         *
    ...
    private static abstract class PhotoDialogBuilderBase<CONCRETE extends PhotoDialogBuilderBase<?>>
         ...
         * dialog, by providing an in-memory representation of the photos. The dialog's callback
         * will be called once the user has shared the photos, but the photos themselves may be
         * uploaded in the background by the Facebook app; apps wishing to be notified when the
         * photo upload has succeeded or failed should extend the FacebookBroadcastReceiver class
         * and register it in their AndroidManifest.xml.</p> <p>In order for the images to be
         * provided to the Facebook application as part of the app call, the
         * NativeAppCallContentProvider must be specified correctly in the application's
         * AndroidManifest.xml.</p> No more than six photos may be shared at a time.
         *
        ...
        /**
         * Adds one or more photos to the list of photos to display in the native Share dialog, by
         * specifying their location in the file system. The dialog's callback will be called once
         * the user has shared the photos, but the photos themselves may be uploaded in the
         * background by the Facebook app; apps wishing to be notified when the photo upload has
         * succeeded or failed should extend the FacebookBroadcastReceiver class and register it in
    ...
    /**
     * Provides a builder which can construct a FacebookDialog instance suitable for presenting the
     * native Share dialog for sharing photos. This builder will throw an exception if the Facebook
     * application is not installed, so it should only be used if {@link
     * FacebookDialog#canPresentShareDialog(android.content.Context, com.facebook.widget.FacebookDialog.ShareDialogFeature...)}
    ...
    /**
     * Provides a builder which can construct a FacebookDialog instance suitable for presenting the
     * native Message dialog for sharing photos. This builder will throw an exception if the
     * Messenger application is not installed, so it should only be used if {@link
    ...
    public static class PhotoMessageDialogBuilder
         ...
         * Constructor.
         *
         * @param activity the Activity which is presenting the native Message dialog; must not be
         *                 null
         */
    ...
    /**
     * Provides a builder which can construct a FacebookDialog instance suitable for presenting the
     * native Message dialog. This builder will throw an exception if the Facebook Messenger
     * application is not installed, so it should only be used if {@link
     * FacebookDialog#canPresentMessageDialog(android.content.Context,
     * com.facebook.widget.FacebookDialog.MessageDialogFeature...)}  indicates the capability is
     * available.
    ...
    private static abstract class OpenGraphDialogBuilderBase<CONCRETE extends OpenGraphDialogBuilderBase<?>>
         ...
         * Constructor.
         *
         * @param activity            the Activity which is presenting the native Open Graph action
         *                            publish dialog; must not be null
         * @param action              the Open Graph action to be published, which must contain a
         *                            reference to at least one Open Graph object with the property
         *                            name specified by setPreviewPropertyName; the action must have
         *                            had its type specified via the {@link OpenGraphAction#setType(String)}
         *                            method
         * @param actionType          the type of the Open Graph action to be published, which
         *                            should be the namespace-qualified name of the action type
         *                            (e.g., "myappnamespace:myactiontype"); this will override the
         *                            type of the action passed in.
         * @param previewPropertyName the name of a property on the Open Graph action that contains
         *                            the Open Graph object which will be displayed as a preview to
         *                            the user
         */
        @Deprecated
         ...
         * method. This method will generate unique names for the image attachments and update the
         * action to refer to these attachments. Note that calling setAction again after calling
         * this method will not clear the image attachments already set, but the new action will
         * have no reference to the existing attachments. The images will not be marked as being
         * user-generated.</p> <p/> <p>In order for the images to be provided to the Facebook
         ...
         * action to refer to these attachments. Note that calling setAction again after calling
         * this method will not clear the image attachments already set, but the new action will
         * have no reference to the existing attachments. The images may be marked as being
         * user-generated -- refer to <a href="https://developers.facebook.com/docs/opengraph/howtos/adding-photos-to-stories/">this
         * article</a> for more information.</p> <p/> <p>In order for the images to be provided to
         * the Facebook application as part of the app call, the NativeAppCallContentProvider must
         * be specified correctly in the application's AndroidManifest.xml.</p>
         *
         ...
         * <p>Specifies a list of images for the Open Graph action that should be uploaded prior to
         * publishing the action. The action must already have been set prior to calling this
         * method.  The images will not be marked as being user-generated. This method will generate
         * unique names for the image attachments and update the action to refer to these
         * attachments. Note that calling setAction again after calling this method will not clear
         * the image attachments already set, but the new action will have no reference to the
         * existing attachments.</p> <p/> <p>In order for the images to be provided to the Facebook
         * application as part of the app call, the NativeAppCallContentProvider must be specified
         * correctly in the application's AndroidManifest.xml.</p>
         *
         * @param bitmapFiles a list of Files containing bitmaps to be uploaded and attached to the
         *                    Open Graph action
         * @return this instance of the builder
         */
         ...
         * <p>Specifies a list of images for the Open Graph action that should be uploaded prior to
         * publishing the action. The action must already have been set prior to calling this
         * method. The images may be marked as being user-generated -- refer to <a
         ...
         * attachments and update the action to refer to these attachments. Note that calling
         * setAction again after calling this method will not clear the image attachments already
         * set, but the new action will have no reference to the existing attachments.</p> <p/>
         ...
         * should be uploaded prior to publishing the action. The images will not be marked as
         * user-generated. The action must already have been set prior to calling this method, and
         * the action must have a GraphObject-valued property with the specified property name. This
         * method will generate unique names for the image attachments and update the graph object
         * to refer to these attachments. Note that calling setObject again after calling this
         ...
         *                       Graph object; the object must be marked as a new object to be
         *                       created (i.e., {@link com.facebook.model.OpenGraphObject#getCreateObject()}
         *                       must return true) or an exception will be thrown
         * @param bitmaps        a list of Files containing bitmaps to be uploaded and attached to
        ...
        /**
         * <p>Specifies a list of images for an Open Graph object referenced by the action that
         * should be uploaded prior to publishing the action. The images may be marked as being
         * user-generated -- refer to <a href="https://developers.facebook.com/docs/opengraph/howtos/adding-photos-to-stories/">this
         * article</a> for more information. The action must already have been set prior to calling
         * this method, and the action must have a GraphObject-valued property with the specified
         * property name. This method will generate unique names for the image attachments and
         * update the graph object to refer to these attachments. Note that calling setObject again
         * after calling this method, or modifying the value of the specified property, will not
         * clear the image attachments already set, but the new action (or objects) will have no
         * reference to the existing attachments.</p> <p/> <p>In order for the images to be provided
         * to the Facebook application as part of the app call, the NativeAppCallContentProvider
         * must be specified correctly in the application's AndroidManifest.xml.</p>
         *
         * @param objectProperty  the name of a property on the action that corresponds to an Open
         *                        Graph object; the object must be marked as a new object to be
         *                        created (i.e., {@link com.facebook.model.OpenGraphObject#getCreateObject()}
     ...
     * it should only be used if {@link FacebookDialog#canPresentOpenGraphActionDialog(android.content.Context,
     * com.facebook.widget.FacebookDialog.OpenGraphActionDialogFeature...)} indicates the capability
     * is available.
     */
    ...
    /**
     * Provides a builder which can construct a FacebookDialog instance suitable for presenting the
     * native Open Graph action message dialog. This builder allows the caller to specify binary
     * images for both the action and any Open Graph objects to be created prior to publishing the
     * action. This builder will throw an exception if the Facebook application is not installed, so
     * it should only be used if {@link FacebookDialog#canPresentOpenGraphMessageDialog(android.content.Context,
    ...
    public static class OpenGraphMessageDialogBuilder
         ...
         *
         * @param activity            the Activity which is presenting the native Open Graph action
         *                            message dialog; must not be null
         * @param action              the Open Graph action to be sent, which must contain a
         *                            reference to at least one Open Graph object with the property
         *                            name specified by setPreviewPropertyName; the action must have
         *                            had its type specified via the {@link OpenGraphAction#setType(String)}
         *                            method
